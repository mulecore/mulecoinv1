...
uint256 hashGenesisBlock("0x9579b5d4ff496f3285b238ec4795cc6b426012113a9c5e6c8acfa9db6afd049a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mulecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mulecoin Signed Message:\n";
...
{
    // Mulecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Mulecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Mulecoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Mulecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mulecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mulecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1aeaed63d76de820a0afd1e8005f07236b2beedb2843ae7b906f37c28dd3a201");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/03/2021 - MuleCoin got new community!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 45 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04256f1721eecae2f90d587ab1cea3035e36444714f28800df83d55ef8cc086d4311a142bca534b33b11ab2b25da1d24c192be2822eec1365b01c9313dcb4d3a16") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1616041312;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084718286;

        if (fTestNet)
        {
            block.nTime    = 1616041288;
            block.nNonce   = 385869770;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x86602e61edc882611c97af3b1bfa63d500ff647d6f66a5e68581d877d9074708"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc5, 0xb7, 0xdf }; // Mulecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MulecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MulecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MulecoinMiner(CWallet *pwallet)
{
    printf("MulecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running MulecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MulecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MulecoinMiner, pwallet));
}
